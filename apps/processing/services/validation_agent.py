...
class ValidationAgent:
    ...
    def validate_record(self, record: dict) -> ValidationResult:
        ...
        ref_issues, ref_confidence = self._validate_referencia(
        ...
        if "fecha_consignacion" in record:
            date_issues, date_confidence = self._validate_fecha(
                record["fecha_consignacion"]
            )
        ...
        if "hora_consignacion" in record:
            time_issues, time_confidence = self._validate_hora(
                record["hora_consignacion"]
            )
    ...
    def _validate_cross_fields(self, record: dict) -> tuple[list, float]:
        ...
        if "fecha_consignacion" in record and record["fecha_consignacion"]:
            try:
                ...
                date_obj = datetime(int(year), int(month), int(day))
                if date_obj > datetime.now() + timedelta(
                    days=1
                ):  # Permitir 1 día de margen
                    confidence -= 0.1
            except:
                pass
