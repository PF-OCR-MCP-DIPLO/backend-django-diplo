...
class ProcessingSupervisorAgent:
    ...
    def process_image(self, process_run, source_image, runtime_config, log_callback):
        ...
        while attempt_number <= self.retry_agent.MAX_RETRIES_PER_IMAGE:
            try:
                ...
                structured_result = self._run_structuring_phase(
                    process_run, source_image, cleaning_result.cleaned_text,
                    current_config, log_callback
                )
                if not structured_result or structured_result.get("error"):
                    raise Exception(f"Structuring failed: {structured_result.get('error')}")
                ...
                if all_valid:
                    ...
                    log_callback(
                        process_run, source_image, "validation_passed",
                        current_config,
                    ...
                    log_callback(
                        process_run, source_image, "validation_failed",
                        current_config,
                    ...
                    if not retry_decision.should_retry:
                        log_callback(
                            process_run, source_image, "retry_decision_no_retry",
                            current_config,
                        ...
                        log_callback(
                            process_run, source_image, "retry_applied",
                            current_config,
                ...
                log_callback(
                    process_run, source_image, f"error_{error_type}",
                    current_config,
                ...
                if not retry_decision.should_retry or attempt_number >= self.retry_agent.MAX_RETRIES_PER_IMAGE:
                    log_callback(
                        process_run, source_image, "error_final",
                        current_config,
                ...
                current_config = self._apply_retry_strategy(current_config, retry_decision)
                attempt_number += 1
        ...
        source_image.save(
            update_fields=["ocr_status", "ocr_raw_text", "ocr_provider", "error_message", "updated_at"]
        )
        ...
        if aggregated:
            log_callback(
                process_run, source_image, "aggregation_summary",
                runtime_config,
                raw_payload={
    ...
    def _run_ocr_phase(self, process_run, source_image, runtime_config, log_callback):
        ...
        try:
            with stage_timer(
                process_run=process_run, source_image=source_image,
                stage="image_validation", runtime_config=runtime_config,
                provider=runtime_config.ocr_provider, model=runtime_config.ocr_model,
            ):
                validate_source_image(source_image)
            
            with stage_timer(
                process_run=process_run, source_image=source_image,
                stage="ocr", runtime_config=runtime_config,
                provider=runtime_config.ocr_provider, model=runtime_config.ocr_model,
            ) as event:
                result = extract_raw_text(source_image, runtime_config)
                event["provider"] = result["provider"]
    ...
    def _run_structuring_phase(self, process_run, source_image, text, runtime_config, log_callback):
        """Fase 3: Extracción LLM."""
        try:
            with stage_timer(
                process_run=process_run, source_image=source_image,
                stage="llm_structuring", runtime_config=runtime_config,
                provider=runtime_config.llm_provider, model=runtime_config.llm_model,
            ) as event:
    ...
    def _apply_retry_strategy(self, config, retry_decision):
        ...
        if retry_decision.strategy == RetryStrategy.CHANGE_OCR_MODE:
            if retry_decision.next_ocr_mode:
                config_copy = replace(config_copy, ocr_mode=retry_decision.next_ocr_mode)
        elif retry_decision.strategy == RetryStrategy.CHANGE_LLM_MODEL:
            if retry_decision.next_llm_model:
                config_copy = replace(config_copy, llm_model=retry_decision.next_llm_model)
    ...
    def _aggregate_and_select_records(self):
        ...
        if aggregated:
            if aggregated.recommendation == "accept":
                ...
                return aggregated.best_attempt.records if aggregated.best_attempt else []
        return []
