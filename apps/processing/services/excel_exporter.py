...
def export_job_to_excel(process_run):
    ...
    for source_image in process_run.source_images.order_by("sequence_index", "id"):
        for deposit in source_image.deposits.order_by("sequence_index", "id"):
            descripcion = description_for_deposit(deposit)

            observations = deposit.observations or []
            if isinstance(observations, list):
                observations_text = "; ".join(str(item) for item in observations)
            else:
                observations_text = str(observations)
